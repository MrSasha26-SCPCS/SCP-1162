...
function SCPHole:Update()
    ...
    if self.main.netEvent.isClient then
        if not created then            
            ...
            if bundle ~= nil and room ~= nil then
                local material = bundle:LoadAsset("scp1162mat.mat")
                local shader = bundle:LoadAsset("shader.shader")
                if material ~= nil and shader ~= nil then
                    ...
                    if plane ~= nil then                
                        ...
                        local meshRenderer = plane:GetComponent(typeof(CS.UnityEngine.MeshRenderer))
